...
function simclick_entry_author( $date = true, $author = true ) {
	...
	if ( $author ) {
		...
		$byline = printf(
			/* translators: %s: post author */
			'<span class="author vcard"><span class="author-label">By </span>%s<a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>', ''
		);
	}
}
endif;
